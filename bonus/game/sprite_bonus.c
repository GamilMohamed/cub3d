...
/*   Updated: 2023/02/16 03:32:20 by mgamil           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_sprite(t_sprite *s, t_drawrays *r, t_plane *p, int time)
{
	while (++s->i < s->de.x)
	{
...
void	init_sprite_draw(t_sprite *s, t_drawrays *r, t_plane *p, int time)
{
	...
	s->ds.y = -s->sprite_h / 2 + HEIGHT / 2 + s->vmove_s;
	if (s->ds.y < 0)
		s->ds.y = 0;
	s->de.y = s->sprite_h / 2 + HEIGHT / 2 + s->vmove_s;
	if (s->de.y >= HEIGHT)
		s->de.y = HEIGHT - 1;
	s->sprite_w = (int)fabs((HEIGHT / s->transform.y) / UDIV);
	s->i = s->ds.x - 1;
	print_sprite(s, r, p, time);
}
...
void	init_sprite(t_map *map, t_drawrays *r, t_plane *p, int time)
{
	...
	while (++s.z < 64)
	{
		...
		s.transform.y = s.invdet * (-p->plane.y * s.sprite.x + p->plane.x
				* s.sprite.y);
		s.spritescreen = (int)((WIDTH / 2) * (1 + s.transform.x
					/ s.transform.y));
		s.vmove_s = (int)(VMOVE / s.transform.y);
		s.sprite_h = (int)fabs((HEIGHT / s.transform.y) / VDIV);
		init_sprite_draw(&s, r, p, time);
	}
}
